...
def first_categories_keyboard(fl_categories):
    ...
    for fl_category in fl_categories:
        keyboard.insert(
            types.InlineKeyboardButton(text=f'{fl_category}', callback_data=f"{fl_categories.index(fl_category)}_fc_btn"))
...
def second_categories_keyboard(sl_categories):
    ...
    for sl_category in sl_categories:
        keyboard.insert(
            types.InlineKeyboardButton(text=f'{sl_category}', callback_data=f"{sl_categories.index(sl_category)}_sc_btn"))
    keyboard_back = types.InlineKeyboardButton(text="Назад", callback_data="back_button")
    keyboard.add(keyboard_back)
    return keyboard
...
def third_categories_keyboard(tl_categories):
    ...
    for tl_category in tl_categories:
        keyboard.insert(
            types.InlineKeyboardButton(text=f'{tl_category}', callback_data=f"{tl_categories.index(tl_category)}_tc_btn"))
    keyboard_back = types.InlineKeyboardButton(text="Назад", callback_data="back_button")
    keyboard.add(keyboard_back)
    return keyboard
